...
export function buildVerifiedProcedure(verify: VerifyToken) {
    ...
    return publicProcedure.use(({ ctx, next }) => {
        if (ctx.authUser) {
            if (ctx.authUser.role === 'user' && ctx.authUser.isRegistered === false) {
                throw new TRPCError({
                    code: 'FORBIDDEN',
                    message:
        ...
        if (authUser.role === 'user' && authUser.isRegistered === false) {
            throw new TRPCError({
                code: 'FORBIDDEN',
                message:
                    'Only verified users have permission to access this resource.',
