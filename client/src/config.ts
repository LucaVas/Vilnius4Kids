...
export const googleMapsMapId = import.meta.env
  .VITE_GOOGLE_MAPS_MAP_ID as string;
export const apiBase = `${apiOrigin}${apiPath}`;
export let filesTypesAllowed = import.meta.env
  .VITE_AWS_S3_FILE_TYPES_ALLOWED as string;
export let maxFileSizeAllowed = import.meta.env
  .VITE_AWS_S3_FILE_SIZE_MAX_IN_BYTES as string;
...
if (typeof googleMapsMapId !== 'string')
  throw new Error('VITE_GOOGLE_MAPS_MAP_ID is not defined');

if (typeof filesTypesAllowed !== 'string')
  filesTypesAllowed = 'image/png,image/jpeg,image/jpg';
